...
class PGAgent(nn.Module):
    def __init__(self, args: Args) -> None:
        ...
        self.size_emb = nn.Embedding(50*50, self.emb_dim)
        self.length_emb = nn.Embedding(50, self.emb_dim)
        self.width_emb = nn.Embedding(50, self.emb_dim)
    ...
    def forward(self, infos: List[pd.DataFrame], encoder_inputs : torch.Tensor, remaining_area = None):
        ...
        for idx in range(self.n_take_in):
            ...
            ####
            len_emb = self.length_emb(torch.ceil(block_size[:, 0].reshape(-1)).type(torch.int))
            wid_emb = self.width_emb(torch.ceil(block_size[:, 1].reshape(-1)).type(torch.int))
            embedded_block = (len_emb * wid_emb).unsqueeze(1)
            
            len_emb = self.length_emb(torch.ceil(self.yard_slots[:, :, :, 0].reshape(-1)).type(torch.int)).reshape(self.batch_size, self.n_yard, -1, self.emb_dim)
            wid_emb = self.width_emb(torch.ceil(self.yard_slots[:, :, :, 1].reshape(-1)).type(torch.int)).reshape(self.batch_size, self.n_yard, -1, self.emb_dim)
            embedded_slot_pre = len_emb * wid_emb
        ...
        for idx in range(self.n_take_out):
            ...
            if len(self.priori_index)  != 0:
                out[:, self.priori_index] = -15000 # 특정 블록을 보내야 하므로 배정된 야드 외에 값들은 제한
            if self.n_trip_infos_list is not None:
                out[self.mask_wrong_barge_selection.transpose(1,2).reshape(self.batch_size, -1)] = -20000 # 최대 항차 수가 넘은 경우 제한 
            out[:, self.n_yard*self.n_take_out:] = -30000 # 블록 개수보다 많을 필요는 없으니 마스킹
            ...
            input0 = torch.cat([embedded_block, embedded_yard], dim=1).reshape(self.batch_size, -1)
            #####
            # print(block_size.shape, slot_info.shape)
            ...
            left_space_after_block = (slot_size[:, 0] * slot_size[:, 1] - (block_size[:, :, 0] * block_size[:, :, 1]).reshape(-1)) ** 2 * (torch.where(isFeasible, 1, -1e-3)) # 불가능한 선택이면 Reward 최소화
            ...
            reward_space = left_space_after_block + torch.sum(torch.sum(remaining_space_of_slots, dim=-1), dim=-1)
            ...
            obj = self.alpha * torch.sum(self.reward_spaces_to, dim=-1) + self.beta * total_barge_num + penalty_for_excessive_barge
            if self.n_trip_infos_list is not None:
                obj = obj + penalty_for_wrong_barge_selection 
            # obj = self.alpha * torch.sum(self.reward_spaces_to, dim=-1) + self.beta * total_barge_num + penalty_for_excessive_barge
...
class RLEnv():
    ...
    def update_policy(self):
        ...
        norm_returns = (take_out_returns - take_out_returns.mean(dim=-1).unsqueeze(-1)) / take_out_returns.std(dim=-1).unsqueeze(-1)
        take_out_loss = torch.mul(-take_out_probs, -norm_returns) # Minimize 
        # take_out_loss = torch.mul(-take_out_probs, norm_returns) # Maximize
        ...
        norm_returns = (take_in_returns - take_in_returns.mean(dim=-1).unsqueeze(-1)) / take_in_returns.std(dim=-1).unsqueeze(-1)
        take_in_loss = torch.mul(-take_in_probs, -norm_returns) # Minimize
        # take_in_loss = torch.mul(-take_in_probs, norm_returns) # Maximize
        ...
        torch.nn.utils.clip_grad_norm_(self.RLAgent.parameters(), 1)
        self.optimizer.step()
    ...
    def load_env(self):
        ...
        self.args.n_trip_infos_list = [self.args.n_trip_infos[k] for k in self.labels_encoder.keys() if k != "사내"] if self.args.n_trip_infos is not None else None
        self.RLAgent = PGAgent(self.args)
        if os.path.exists(self.args.weight_dir) and self.args.do_load_weights:
            self.RLAgent.load_state_dict(torch.load(args.weight_dir))
        self.RLAgent.to(self.args.device)
...
def save_result(result, args, infos, labels_encoder, obj_loss_history=None, dl_loss_history=None):
    ...
    result = result.sort_values(by=["목적지", "바지", "슬롯"])
...
def train(args : Args):
    args.do_load_weights = True
    for n in tqdm(range(args.n_exp)):
        
        input_dir = os.path.join(exp_dir, f"Exp_{n}", "Input")
        args.input_dir = input_dir
        os.makedirs(input_dir, exist_ok=True)
        
        data_dir = os.path.join(exp_dir, f"Exp_{n}", "Temp")
        args.data_dir = data_dir
        os.makedirs(data_dir, exist_ok=True)
        
        args.save_dir = os.path.join(exp_dir, f"Exp_{n}", "Result")
        ...
        # n_trip_infos = pd.read_csv(os.path.join(args.input_dir, "shi_max_trip_count.csv"), encoding="cp949")
        ...
        with open(os.path.join(args.save_dir, "result.txt"), "w") as f:
            ...
            try:
                ...
                save_result(obj_loss_history=obj_loss_history, dl_loss_history=dl_loss_history, result=kwargs, args=args, infos=(take_in, take_out), labels_encoder=env.labels_encoder_inv)
                torch.save(env.RLAgent.state_dict(), args.weight_dir)
...
def inference(args : Args):
    def inference_single(exp_num):
        input_dir = os.path.join(exp_dir, f"Exp_{exp_num}", "Input")
        args.input_dir = input_dir
        os.makedirs(input_dir, exist_ok=True)
        
        data_dir = os.path.join(exp_dir, f"Exp_{exp_num}", "Temp")
        args.data_dir = data_dir
        os.makedirs(data_dir, exist_ok=True)
        
        args.save_dir = os.path.join(exp_dir, f"Exp_{exp_num}", "Result_infer")
        os.makedirs(args.save_dir, exist_ok=True)
        
        with open(os.path.join(args.save_dir, "result.txt"), "w") as f:
            f.write("case,objective,time\n")
            for do_rough_guess in [True, False]:
                args.do_load_weights = False if do_rough_guess else True
                ...
                with torch.no_grad():
                    ...
                    time2 = time.time()
                    kwargs = env.get_result()
                    result = save_result(result=kwargs, args=args, infos=(take_in, take_out), labels_encoder=env.labels_encoder_inv)
                    result.to_csv(os.path.join(args.save_dir, f"result_{'rough' if do_rough_guess else 'weight'}.csv"), encoding="cp949", index=False)
                    torch.save(env.RLAgent.state_dict(), args.weight_dir)
                    

                    print(f"Took {(time2 - time1):.3f} seconds")
                    # f.write(f"{kwargs['take_out_rewards']},{time2-time1}\n")
                    f.write(f"{'rough' if do_rough_guess else 'weight'},{kwargs['take_out_rewards'][-1]:.3f},{(time2-time1):.3f}\n")
                    
    if isinstance(args.exp_num, list):
        for exp_num in args.exp_num:
            inference_single(exp_num)
    elif isinstance(args.exp_num, int):
        inference_single(args.exp_num)
    else:
...
def generate_empty_slots(args, mapping_dict):
    Slot_dict = {
        "HSG성동" : {
            0: (25, 22, 84)
        },
    ...
    empty_slots_dfs = []
    for name, slot_capa in Slot_dict.items():
        for case, capa in slot_capa.items():
            max_length, max_width, max_count = capa
            n_rand_slots = np.random.randint(0, max(int(max_count * 0.3), 1))
        
            empty_slots = np.zeros((n_rand_slots, 3))
            empty_slots[:, 0] = np.random.randint(int(max_length * 0.8), max_length, n_rand_slots)
            empty_slots[:, 1] = np.random.randint(int(max_width * 0.8), max_width, n_rand_slots)
            # empty_slots[:, 2] = np.round(np.random.uniform(2, 6, n_rand_slots), 2)
            # empty_slots[:, 2] = np.random.randint(2, 6, n_rand_slots)
            if name == "봉암8" or name == "봉암9":
                empty_slots[:, 2] = 5.8
            elif name == "덕곡2":
                empty_slots[:, 2] = 5
            else:
            ...
            empty_slots_df["location"] = name
            empty_slots_dfs.append(empty_slots_df)
    
    empty_slots_dfs = pd.concat(empty_slots_dfs)
    empty_slots_dfs.to_csv(os.path.join(args.input_dir, "DB_empty_slots.csv"), index=False, encoding="cp949")
    
def generate_max_trips(args):
    yard_names = ["HSG성동","덕곡1","덕곡2","봉암8","봉암9","신한내","오비","오비3","한내"]
    max_trips = []
    for name in yard_names:
        n_rand_slots = np.random.randint(0, 4)
        max_trips.append((name, n_rand_slots))
    
    max_trips_df = pd.DataFrame(max_trips, columns=["name", "count"])
    # print(max_trips_df)
    max_trips_df.to_csv(os.path.join(args.input_dir, "shi_max_trip_count.csv"), index=False, encoding="cp949")
...
def generate_dataset(args):
    df_db_block = pd.read_csv(os.path.join(args.orig_dir, "DB_whole_block.csv"), encoding="cp949")
    ...
    area_dict = pd.read_csv(os.path.join(args.src_dir, "area_dict.csv"), encoding="cp949")
    ...
    whole_block.to_csv(os.path.join(args.data_dir, "temp_whole_block.csv"), encoding="cp949", index=False)
        
    # 반출 블록 랜덤 선정
    n_rand_take_out = np.random.randint(5, 30)
    blocks_at_in = whole_block.loc[(whole_block["location"] == "사내")].copy()
    blocks_at_in = blocks_at_in.iloc[:n_rand_take_out]
    blocks_at_in.to_csv(os.path.join(args.data_dir, "temp_take_out.csv"), encoding="cp949", index=False)
        
    # 반입 블록 랜덤 선정
    n_rand_take_in = np.random.randint(5, 30)
    blocks_at_out = whole_block.loc[(whole_block["location"] != "사내")].copy()
    blocks_at_out = blocks_at_out.iloc[:n_rand_take_in]
    blocks_at_out.to_csv(os.path.join(args.data_dir, "temp_take_in.csv"), encoding="cp949", index=False)
    
    # 임의로 빈 슬롯 생성
    generate_empty_slots(args, mapping_dict) 
    generate_max_trips(args)
        
    # 사용자가 남아있는 슬롯에 대해 기록을 할 때, count로 기록된 수만큼 저장하는 함수
    empty_slots = pd.read_csv(os.path.join(args.input_dir, "DB_empty_slots.csv"), encoding="cp949")
    ...
    for loc in whole_block["location"].unique():
        ...
        if empty_slots.loc[empty_slots["location"] == loc].empty:
            ...
            empty_location_slot_extended = pd.DataFrame(np.concatenate(empty_location_slot_extended, axis=0), columns=["vessel_id", "block", "length", "width", "height", "weight"])
            yard_slots = pd.concat([location_slots, empty_location_slot_extended])
            
        yard_slots.to_csv(os.path.join(args.data_dir, f"{loc}.csv"), encoding="cp949", index=False)
...
if __name__ == "__main__":
    ...
    exp_dir = os.path.join(par_dir, "Exps_test")
    args.exp_dir = exp_dir
    args.orig_dir = os.path.join(par_dir, "Input")
...
temp = torch.arange(24).reshape(4,3,2)
# temp[:, 4]
# %%
