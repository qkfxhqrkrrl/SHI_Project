...
def sample_dataset():
    ...
    blocks_to_move = result.iloc[:50].copy()
    blocks_to_stay = result.iloc[50:].copy()
    
    n_block_out = np.random.randint(300, 350)
    block_out = blocks_to_stay.sample(n_block_out)
    block_out.loc[:, "location"] = "사외"
    block_in = blocks_to_stay.loc[~blocks_to_stay.index.isin(block_out.index), :]
    block_in.loc[:, "location"] = "사내"
    
    n_take_out = np.random.randint(20, 30)
    n_take_in = 50 - n_take_out
    blocks_to_be_out = blocks_to_move.sample(n_take_out)
    blocks_to_be_out["location"] = "사내"
    blocks_to_be_in = blocks_to_move.loc[~blocks_to_move.index.isin(blocks_to_be_out.index), :]
    # print(blocks_to_be_in)
    blocks_to_be_in.loc[:, "location"] = "사외"
    
    result = pd.concat([blocks_to_be_in, blocks_to_be_out, block_in, block_out], axis=0)
    
    return result.reset_index(drop=True), blocks_to_be_in, blocks_to_be_out
...
def decide_blocks(env, in_range : Tuple = (20, 30), out_range: Tuple = (20, 30)):
    ...
    sampled_blocks = sizes.iloc[:50].copy()
    result = convert_size(sampled_blocks)
    rename_dict = {"PROJ_NO" : "vessel_id", "BLK_NO": "block", "LTH": "length", "BTH": "width", "NET_WGT": "weight", "HGT": "height"}
    result.rename(columns=rename_dict, inplace=True)
    result = result[rename_dict.values()]
    
    blocks_to_be_out = result.sample(n_take_out)
    blocks_to_be_in = result.loc[result.index.difference(blocks_to_be_out.index)]
...
class PGAgent_to(nn.Module):
    def __init__(self, n_yard_out=10, n_max_block=60, hidden_dim = 256, device = device, batch_limit = (65,20), n_take_out = None) -> None:
        ...
        self.emb_yard = nn.Embedding(n_yard_out, 5*(pad_len+1))
    ...
    def generate_mask(self, take_out: pd.DataFrame, yard_slots: np.ndarray):
        ...
        for b_idx, block_info in enumerate(take_out[["length", "width", "height"]].values):
            for y_idx in range(self.n_yard):
                mask = np.any(np.all(block_info <= yard_slots[:, y_idx*pad_len: (y_idx+1)*pad_len, :3], axis=-1), axis=-1) # 블록 크기가 슬롯보다 작으면 True
...
class RLEnv():
    ...
    def __init__(self, whole_block: pd.DataFrame) -> None:
        ...
        self.whole_block : pd.DataFrame = whole_block
    ...
    def _init_env(self):
        def _init_yard_out(info):
            ...
            for _, slot_info in info.iterrows():
                ...
                # rand_blocks = pd.concat([rand_blocks, self.whole_block.loc[self.whole_block["RANK"], :]], axis=1)
        ...
        self.whole_block.loc[self.whole_block["location"] == "사외", "location"] = "사내"
...
def run():
    ...
    # exp_dir = os.path.join(result_dir, f"Debug")
    os.makedirs(exp_dir, exist_ok=True)
    
    # load_dir = os.path.join(result_dir, "2024_3_4_17_48_dataset")
    ...
    with open(os.path.join(exp_dir, "result.txt"), "w") as f:
        f.write("exp_num,objective,time\n")
        # for exp_num in tqdm(range(10)):
        whole_block, take_in, take_out = sample_dataset()
        for exp_num in tqdm(range(1)):
            env = RLEnv(whole_block)
            ...
            new_take_in = []
            new_take_out = []
            for idx, row in take_in.iterrows():
                location = env.whole_block.loc[(env.whole_block["vessel_id"] == row["vessel_id"]) & (env.whole_block["block"]==row["block"]), "location"].values[0]
                if location != "사내":
                    row["location"] = location
                    new_take_in.append(row)
                else:
                    row["location"] = "사내"
                    new_take_out.append(row)
            
            new_take_in = pd.DataFrame(new_take_in)
            new_take_out = pd.DataFrame(new_take_out)
            new_take_out = pd.concat([new_take_out, take_out], axis=0)
            
            encoder_inputs, yard_slots, remaining_areas = env.get_state(new_take_in.copy(), new_take_out.copy())
            ...
            env.save_env(exp_dir, exp_num, (new_take_in, new_take_out, yard_slots))
            ...
            f.write(f"{exp_num},{result_obj},{time2-time1}\n")
            # break
...
def temp_dataset():
    data_dir = r"D:\Dropbox\Projects_Mine\삼성중공업\code\SHI_Project\Result\data"
    csv_paths = glob(os.path.join(data_dir, "*.csv"))
    print(csv_paths)
    
    df_ti = pd.read_csv(os.path.join(data_dir, "0_take_in.csv"))
    df_to = pd.read_csv(os.path.join(data_dir, "0_take_out.csv"))
    
    block_plan = pd.concat([df_ti, df_to]).reset_index(drop=True)
    block_plan.to_csv(os.path.join(data_dir, "block_plan.csv"), index=False, encoding="cp949")
    
    slots = []
    for path in csv_paths:
        if path in [os.path.join(data_dir, "0_take_in.csv"), os.path.join(data_dir, "0_take_out.csv")]:
            continue
        
        df = pd.read_csv(path)
        name = path.replace(os.path.join(data_dir, ""), "").split("_")[-1].replace(".csv", "")
        print(name)
        # display(df)
        count = df.loc[~df["vessel_id"].isnull(),  ["length", "width", "height"]].groupby(["length", "width"], as_index=False).value_counts()
        count["name"] = name
        # display(count)
        slots.append(count)
    slots = pd.concat(slots)
    slots.to_csv(os.path.join(data_dir, "slots.csv"), index=False, encoding="cp949")
    # display(df_ti)
    # display(df_to)
    
    # df_slot = pd.read_csv(os.path.join(data_dir, "0_slot_infos.csv"))
