...
class PGAgent_to(nn.Module):
    def __init__(self, n_yard_out=10, n_max_block=60, hidden_dim = 256, device = device, batch_limit = (65,20), n_take_out = None, emb_dim = 16) -> None:
        ...
        self.decoder = nn.LSTMCell(input_size=emb_dim, hidden_size=hidden_dim)
        ...
        self.emb_lin = nn.Linear(n_max_block+n_yard_out*pad_len, emb_dim)
    ...
    def generate_encoder_input(self, take_out : pd.DataFrame):
        yard_emb = self.yard_emb(torch.arange(self.n_yard).to(self.device))
        emb_idx = torch.LongTensor(25 * np.ceil(take_out["length"]).astype(int) + np.ceil(take_out["width"]).astype(int), d)
        # print(emb_idx)
    ...
    def embed_input(self, block_emb_info : np.array, slot_info):
        ...
        """
        input0 = torch.concat([torch.FloatTensor(block_emb_info), torch.concat(torch.FloatTensor(slot_info), axis=1)], axis=1)
        print(input0.shape)
        quit()
    ...
    def forward(self, infos: List[pd.DataFrame], remaining_area: np.array):
        ...
        input0 = torch.zeros(batch_size, self.emb_dim).to(self.device)
...
def run():
    ...
    with open(os.path.join(exp_dir, "result.txt"), "w") as f:
        ...
        for exp_num in tqdm(range(10)):
            ...
            # (n_take_out, n_take_in), (take_in, take_out, yard_slots), remaining_areas = decide_blocks(env, in_range=(11, 12), out_range=(11, 12)) 
