...
def decide_blocks(env, in_range : Tuple = (30, 60), out_range: Tuple = (30, 60)):
    ...
    yard_slots, remaining_areas = env.get_state(blocks_to_be_out.copy(), blocks_to_be_in.copy())
    ...
    return (n_take_out, n_take_in), (take_in, take_out, yard_slots), remaining_areas

def make_input_from_loaded(blocks_to_be_out, blocks_to_be_in, env):
    # encoder_inputs, yard_slots, remaining_areas = env.get_state(blocks_to_be_out.copy(), blocks_to_be_in.copy())
    yard_slots, remaining_areas = env.get_state(blocks_to_be_out.copy(), blocks_to_be_in.copy())
    return remaining_areas, yard_slots
...
class PGAgent_to(nn.Module):
    def __init__(self, n_yard_out=10, n_max_block=60, hidden_dim = 256, device = device, batch_limit = (65,20), n_take_out = None, emb_dim = 16) -> None:
        super(PGAgent_to, self).__init__()
        self.n_yard = n_yard_out
        self.n_max_block = n_max_block
        self.n_take_out = n_take_out
        self.encoder = nn.LSTM(input_size=emb_dim*2, hidden_size=hidden_dim, batch_first=True)
        self.decoder = nn.LSTMCell(input_size=emb_dim*(emb_dim+1), hidden_size=hidden_dim)
        ...
        self.yard_emb = nn.Embedding(n_yard_out, emb_dim)
        self.size_emb = nn.Embedding(25*25, emb_dim)
    ...
    def generate_mask(self, take_out: pd.DataFrame, yard_slots: np.ndarray):
        ...
        for b_idx, block_info in enumerate(take_out[["length", "width", "height"]].values):
            for y_idx in range(self.n_yard):
                mask = np.any(np.all(block_info <= yard_slots[:, y_idx, :, :3], axis=-1), axis=-1) # 블록 크기가 슬롯보다 작으면 True
                encoder_mask[:, b_idx, y_idx] = mask # 블록이 슬롯보다 작은게 하나라도 있다면 True
        
        return torch.BoolTensor(~encoder_mask)
    ...
    def embed_input(self, block_emb_info : np.array, slot_info):
        ...
        yard_emb = self.yard_emb(torch.arange(self.n_yard).to(self.device)).unsqueeze(0).repeat(batch_size, 1, 1)
        
        embedded_yard = torch.matmul(embedded_slot, yard_emb)
        
        input0 = torch.cat([embedded_block, embedded_yard], dim=1)
        return input0.reshape(batch_size, -1)
        
        
    
    def forward(self, infos: List[pd.DataFrame], remaining_area: np.array):
        """
        Feature information
        ...
        input0 = torch.zeros(batch_size, self.emb_dim * (self.emb_dim + 1)).to(self.device)
        hids, h0, c0 = hids.repeat(batch_size, 1, 1), h0.repeat(batch_size, 1), c0.repeat(batch_size, 1)
        
        block_mask = torch.zeros((batch_size, self.n_take_out, 1), dtype=bool, requires_grad=False) # 제외 시키고 싶은 경우를 True
        encoder_mask = self.generate_mask(take_out, yard_slots) # [Batch, N_take_out, N_yard], True가 제외
        ...
        for idx in range(self.n_take_out):
            ...
            slot_info = yard_slots[range(batch_size), yard_selection] # [B x pad_len x N_feat]
...
class RLEnv():
    ...
    def get_state(self, possible_take_out: pd.DataFrame, take_in: pd.DataFrame):
        ...
        for name, yard in self.yards_out.items():
            ...
            state_part_[["length", "width"]] = state_part_[["length", "width"]].astype(int) # 버림 형식
        ...
        return yard_slots, remaining_areas
            
    def step(self, infos: List[pd.DataFrame], remaining_areas: np.array):
        """
        ...
        probs, rewards, actions, obj  = self.Agent_to((take_in.reset_index(drop=True), take_out.reset_index(drop=True), yard_slots), remaining_areas)
    ...
    def save_env(self, save_dir, exp_num, infos):
        ...
        pd.DataFrame(np.concatenate(yard_slots)).to_csv(os.path.join(save_dir, f"{exp_num}_yard_slots.csv"), index=False)
...
def run():
    ...
    exp_dir = os.path.join(result_dir, f"{current_dateTime.year}_{current_dateTime.month}_{current_dateTime.day}_{current_dateTime.hour}_{current_dateTime.minute}_emb")
    ...
    with open(os.path.join(exp_dir, "result.txt"), "w") as f:
        ...
        for exp_num in tqdm(range(100)):
        # for exp_num in tqdm(range(1)):
            env = RLEnv()
            
            # take_in, take_out= env.load_env(load_dir, exp_num)
            # remaining_areas, yard_slots = make_input_from_loaded(take_out, take_in, env)
            
            (n_take_out, n_take_in), (take_in, take_out, yard_slots), remaining_areas = decide_blocks(env, in_range=(30, 60), out_range=(30, 60)) 
            ...
            for epo in loop:
                ...
                for day in range(1):
                    ...
                    objective = env.step((take_in.copy(), take_out.copy(), deepcopy(yard_slots)), deepcopy(remaining_areas))
...
def temp_emb():
    # temp = pd.DataFrame(np.arange(0, 3, 0.01))
    # temp = pd.DataFrame()
    # temp["int"] = temp.astype(int)
    print(np.arange(0, 3, 0.1))
    print(np.ceil(np.arange(0, 3, 0.1)))
    # display(temp)
    # 25 * 25 * 3
    # sizes = pd.read_csv(os.path.join(src_dir, "block_size.csv")) 
    # display(sizes)
